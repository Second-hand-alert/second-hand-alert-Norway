(function (factory) {
  ...
  document.body.addEventListener('keyup', (event) => {
    setTimeout(() => {
      if (event.key === 'enter' && productUrlCheck(regexReimaProductPage, window.location.href) && firstTime === false && newURL(window.location.href) === true) {
        console.log('###location changed: ' + window.location.href);
        let prodObj = prodObjReima();
        prodObj = extractAndPrepareReima(prodObj);
        sendObj(prodObj);
      }
    }, 1200);
  });
  ...
  document.body.addEventListener('click', (event) => {
    setTimeout(() => {
      ...
      if (productUrlCheck(regexReimaProductPage, window.location.href) && firstTime === false && newURL(window.location.href)) {
        console.log('### location changed: ' + window.location.href);
        let prodObj = prodObjReima();
        prodObj = extractAndPrepareReima(prodObj);
        sendObj(prodObj);
  ...
  if (firstTime === true) {
    firstTime = false;
    if (productUrlCheck(regexReimaProductPage, window.location.href)) {
      setTimeout(() => {
        let prodObj = prodObjReima;
        prodObj = extractAndPrepareReima(prodObj);
        sendObj(prodObj);
